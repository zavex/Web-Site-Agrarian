...
<nav class="navbar navbar-inverse">
  <div class="container-fluid">
    ...
    <section>
    <?php
        if(isset($_POST['idCliente']) && isset($_POST['password'])) {
            include("conectar.php");
            $query = "select * from clientes where idCliente='".$_POST['idCliente']."' and password='".md5($_POST['password'])."'";

            if ($resultado = mysqli_query($conexion_db, $query)) {
                $contar = mysqli_num_rows($resultado);
                if($contar>0){
                    session_start();
                    $_SESSION['usuario']=$_POST['idCliente'];
                    ...
                    echo "<strong>Datos incorrectos!</strong> Id de Cliente y/o contraseña equivocada, intente de nuevo.</div>";
                }
            }
        }
        ?>
